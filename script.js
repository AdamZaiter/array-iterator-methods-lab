...
// Just pasted this from the lab on canvas.
...
// Sort method to sort the inventors by birth year.
// compare the year of birth of tow inventors at the same time.
// return negative if first was born before second, and psoitive if after, and nothing if born in the same year.
...
// Find method to find an inventor with titel Ada
// The find method will return the first element that meets the condition 
...
// Map method to iterate through the array (people)
// Split string in to first and last name. let firstLast = people.map(person)
// rearrage the split in to first and last format "Let" last, first
// return $first $last to return in that format. 
...
// Some method to check if at least one was older than 18
// Calculate the rest of the ages but subtracting birth from current years.
// If >1 is >18 then true.
...
// similar to step 6, but now we want to see if everyone is older than 19
// Use math to calulcate the age
// if they're all older than 19, it'll print true. 
...
// Find method to find the comment ID 823423
// returns first element that follows the coniditon. 
// New variable commentById is gonna contain it.
// Prints super good
...
// like in step 8, but here i used findIndex to find the comment's Index for comment 123523
// New variable idx 
// will show the first one that satisfied the conditions. 
// Prints -3 
